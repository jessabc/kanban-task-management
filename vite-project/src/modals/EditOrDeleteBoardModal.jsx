...
export default function EditOrDeleteBoardModal({isEditDeletBoardModalVisible, setIsEditDeletBoardModalVisible}) {
  ...
  useOnClickOutside(ref, () => setIsEditDeletBoardModalVisible(false));
    ...
    return (
        <>
{/* overlay */}
        <div className={`${isEditDeletBoardModalVisible ? ' fixed top-0 left-0 w-screen h-screen bg-opacity-50 bg-gray-600  flex items-start justify-center ' : ''}`}>

       

            <div className={`${isEditDeletBoardModalVisible ? "flex flex-col absolute right-7 top-20 w-fit py-5 pl-5 pr-10  text-lg bg-gray-50 shadow-md  gap-1  rounded-lg  text-gray-400" : "hidden"}  ` } ref={ref}>
